...
import {
  actions as AtlasFoundationActions,
  AtlasFoundationDocument,
  reducer as AtlasFoundationReducer,
} from "../../document-models/atlas-foundation";
import {
  actions as AtlasScopeActions,
  AtlasScopeDocument,
  reducer as AtlasScopeReducer,
...
const addFoldersAndDocuments = async (
  ...
  for (const key in scopes) {
    ...
    document = AtlasScopeReducer(
      ...
      AtlasScopeActions.populateScope({
        ...
        masterStatus: scope.masterStatus
          .map((s: any) => getMasterStatus(s.id))
          .flat(),
        globalTags: ["CAIS"],
    ...
    const result = await driveServer.queueOperations(
      driveName,
      scope.id,
      document.operations.global.slice(-1)
    );
    const documentResult: AtlasScopeDocument =
      result.document as AtlasScopeDocument;
...
async function main() {
  ...
  driveServer.on("syncStatus", async (driveId, syncStatus) => {
    ...
    await driveServer.addDriveAction(
...
const addArticles = async (
  ...
  for (const articleObj of articleIds) {
    ...
    if (!article) {
      console.warn(
        `  Article not found: ${articleObj.id} for scope ${scope.docNoString}`
      );
      continue;
    }
    ...
    await driveServer.queueDriveAction(
      driveName,
      actions.addFolder({
...
const populateArticle = async (
  ...
  if (document) {
    ...
    await sleep(200);
    const documentResult: AtlasFoundationDocument =
      result.document as AtlasFoundationDocument;
